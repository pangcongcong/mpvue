...
/* harmony default export */ __webpack_exports__["a"] = ({
	data: function data() {
		return {
			...
			shareImgBtn: ['https://s2.wandougongzhu.cn/s/7d/__1_de53f5.jpg', 'https://s.wandougongzhu.cn/s/c8/__2_209e10.jpg', 'https://s5.wandougongzhu.cn/s/58/__3_b21b66.jpg', 'https://s.wandougongzhu.cn/s/10/__4_6eaacb.jpg', 'https://s5.wandougongzhu.cn/s/41/__5_4165fc.jpg'], //分享花瓣给好友图了list
	...
	onLoad: function onLoad(option) {
		...
		console.log(option);
		// this.invite_sakura = option.share_sakura;
		// this.invite_user_id = option.user_id;
	},
	...
	methods: {
		bindViewTap: function bindViewTap() {
			...
			WMP.checkAuthPromise(this).then(function () {
				_this2.userInfo = WMP.globalData.userInfo;
				_this2.user_id = _this2.userInfo.user_id;
				_this2.getUserActInfo();
			});
		},
		getUserActInfo: function getUserActInfo() {
			...
			if (this.lock) {
				Net.get('Sakura.getUserActivityInfoByUserId', {
					...
					if (res.errno == '0') {
						_this3.taskList = res.data.day_task;
						var infoList = res.data.info;
						_this3.sakura_A_num = infoList.sakura_a;
						_this3.sakura_B_num = infoList.sakura_b;
						_this3.sakura_C_num = infoList.sakura_c;
						_this3.sakura_D_num = infoList.sakura_d;
						_this3.sakura_E_num = infoList.sakura_e;
						_this3.sakura_W_num = infoList.sakura_w;
						_this3.sakura_H_num = parseInt(infoList.sakura_h);
						_this3.sakura_G_num = infoList.sakura_g;
						if (infoList.sakura_a > 0 && infoList.sakura_b > 0 && infoList.sakura_c > 0 && infoList.sakura_d > 0 && infoList.sakura_e > 0) {
							_this3.is_compose = true;
						} else {
							_this3.is_compose = false;
						}
						_this3.rest_count = infoList.residue_lottery_count;
						// this.user_id = infoList.user_id;
						_this3.totalSakuraNum = res.data.h_sakura_count;
						_this3.inviteCount = res.data.invite_count;
		...
		onShareAppMessage: function onShareAppMessage(res) {
			if (res.from == 'button') {
				var shareImgBtnImg = void 0;
				console.log('/pages/index/index?share_sakura=' + this.sakuraDetailName + '&user_id=' + this.user_id);
				return {
					title: '送你一朵' + this.sakuraDetailName + '，快来和我平分100万现金！',
					path: '/pages/index/index?share_sakura=' + this.sakuraDetailName + '&user_id=' + this.user_id,
					imageUrl: shareImgBtn[0],
					success: function success(res) {
						wx.navigateTo({ url: '/pages/index/index?share_sakura=' + this.sakuraDetailName + '&user_id=' + this.user_id });
					},
					fail: function fail() {},
		...
		lotterySakura: function lotterySakura() {
			...
			Net.post('Sakura.userLotterySakura', {
				...
				if (res.errno == '0') {
					_this5.sakuraDetailShow = false;
					_this5.sakuraAllChangeShow = false;
					_this5.composeShow = false;
					_this5.sakuraAllShow = false;
					_this5.inviteSakuraAllShow = false;
					_this5.giftRecShow = false;
					_this5.maskShow = true;
					_this5.lotteryResShow = true;

					_this5.lottery_res = res.data;
				} else {
					wx.showModal({ title: '提示', content: res.errmsg });
				}
			});
		...
		handleOrient: function handleOrient() {
			if (window.DeviceOrientationEvent) {
				window.addEventListener('deviceorientation', this.handleOrientation(event), false);
			} else {
...
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: "name"
  }, [_vm._v(_vm._s(_vm.lottery_res.skura_name))])]), _vm._v(" "), _c('div', {
    ...
    }, [(item.type == '1') ? _c('div', [_c('div', {
      staticClass: "rec"
    }, [_vm._v("赠送的"), _c('span', [_vm._v(_vm._s(item.sakura_name))]), _vm._v("被"), _c('span', [_vm._v(_vm._s(item.from_user_nickname))]), _vm._v("领取了")])]) : _vm._e(), _vm._v(" "), (item.type == '2') ? _c('div', [_c('div', {
      staticClass: "rec"
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_6b277275_hasScoped_true_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_index_vue__ = __webpack_require__(34);
var disposed = false
